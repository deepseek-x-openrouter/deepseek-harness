...
import { ASIDE_COLLAPSED, ASIDE_DEFAULT, SIDEBAR_COLLAPSED } from '@deepseek-ai/dsh-client-ui-layout/src/client/columns.ts'
...
function mountFrame() {
  ...
  const renderSlot = ((key: string, owner: object) => {
    ...
    if (key === 'aside') return <div data-testid="aside-content" />
...
describe('AppFrame — aside column', () => {
  it('unoccupied: no track, no rail, seam-suppression attribute set', () => {
    const { frame, getByTestId } = mountFrame()
    expect(tracks(frame)[2]).toBe(0)
    expect(frame.hasAttribute('data-aside-absent')).toBe(true)
    // Mounted at zero width like details — never unmounted.
    expect(getByTestId('aside-content')).toBeTruthy()
  })

  it('occupied: opens at the store preference with live owner props and a handle', () => {
    const { frame, instance, slotCalls } = mountFrame()
    act(() => { instance.actions.setAsideOccupied(true) })
    expect(tracks(frame)).toEqual([280, 0, ASIDE_DEFAULT])
    expect(frame.hasAttribute('data-aside-absent')).toBe(false)
    expect(slotCalls.filter(c => c.key === 'aside').at(-1)!.props)
      .toEqual({ collapsed: false, width: ASIDE_DEFAULT })
    expect(frame.querySelectorAll('[class*="handle"]')).toHaveLength(2)
  })

  it('toggleAside collapses to the fixed rail (no handle) and back', () => {
    const { frame, instance, slotCalls } = mountFrame()
    act(() => { instance.actions.setAsideOccupied(true) })
    act(() => { instance.actions.toggleAside() })
    expect(tracks(frame)).toEqual([280, 0, ASIDE_COLLAPSED])
    expect(slotCalls.filter(c => c.key === 'aside').at(-1)!.props)
      .toEqual({ collapsed: true, width: ASIDE_COLLAPSED })
    expect(frame.querySelectorAll('[class*="handle"]')).toHaveLength(1)
    act(() => { instance.actions.toggleAside() })
    expect(tracks(frame)).toEqual([280, 0, ASIDE_DEFAULT])
  })

  it('aside drag widens leftward (negative dx grows the panel)', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.setAsideOccupied(true) })
    const handles = frame.querySelectorAll('[class*="handle"]')
    drag(handles[1]!, 1500, 1400)
    expect(tracks(frame)[2]).toBe(ASIDE_DEFAULT + 100)
  })

  it('details handle sits left of the aside track', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.setAsideOccupied(true) })
    act(() => { instance.actions.openDetails() })
    const handles = frame.querySelectorAll('[class*="handle"]')
    expect(handles).toHaveLength(3)
    const detailsHandle = frame.querySelector('[data-side="details"]') as HTMLElement
    expect(detailsHandle.style.left).toBe(`${1920 - 360 - ASIDE_DEFAULT}px`)
    const asideHandle = frame.querySelector('[data-side="aside"]') as HTMLElement
    expect(asideHandle.style.left).toBe(`${1920 - ASIDE_DEFAULT}px`)
  })

  it('a blank current session hides the occupied aside entirely', () => {
    const { frame, instance, rerenderFrame } = mountFrame()
    act(() => { instance.actions.setAsideOccupied(true) })
    expect(tracks(frame)[2]).toBe(ASIDE_DEFAULT)
    selectedSessionBlank.current = true
    act(() => { rerenderFrame() })
    expect(tracks(frame)[2]).toBe(0)
  })
