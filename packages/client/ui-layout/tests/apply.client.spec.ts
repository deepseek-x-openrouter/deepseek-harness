...
describe('ui-layout client apply', () => {
  ...
  it('provides ctx.layout and registers AppFrame into root with the child declarations', async () => {
    ...
    expect(slots.spec('aside')).toEqual({ kind: 'single', scope: 'session' })
  })
  ...
  it('injects no business face and attaches the layout actions', async () => {
    ...
    await fiber.await()
    const actions = {
      setSidebar: vi.fn(), setDetails: vi.fn(), setAside: vi.fn(), toggleSidebar: vi.fn(), toggleAside: vi.fn(),
      setAsideOccupied: vi.fn(), openDetails: vi.fn(), closeDetails: vi.fn(),
    }
