...
Shell plugin: column AppFrame (sidebar | conversation | details | aside; drag handles and concession chain) plus the `ctx.layout` panel-geometry service; it registers into the runtime-owned `root` slot and declares `sidebar`, `conversation`, `details`, `aside`, and `conversation.empty`. The sidebar resize boundary is an invisible hit strip, while the details and aside boundaries retain floating pills; concession shrinks then auto-closes details; an open aside has a floor but no width ceiling — it holds its dragged width while center absorbs (down to zero), trimming only when the preference exceeds the whole non-sidebar span (never below its rail). A closed sidebar retains a 56px control rail while details closes to zero width; the aside is a plugin-claimable mirror of the sidebar — absent without an occupant, a slim rail when collapsed (the occupant renders the rail UI), toggled via `ctx.layout.toggleAside()`. The package also seats the theme presenter: it consumes resolved `ctx.theme` snapshots and projects them onto the document (`html { color-scheme }` for native UA chrome, `body[data-ds-dark-theme]` from the active color scheme, the theme's alias tokens as inline variables on body, and one owned `<meta name="theme-color">` whose content follows the computed body background). Measuring after palette and token application keeps the rendered background as the single color authority; disposing the presenter removes its metadata node with its other global writes.
